...
body
    The second part of a compound statement. The body consists of a sequence of 
    statements all indented the same amount from the beginning of the header.
    The standard amount of indentation used within the Python community is 4
    spaces.
...
import
    ...
    Python shell.For example, assume the following is in a script named
    ``tryme.py``:
    ...
    If we import everything from ``tryme.py``, however, we can use everything
    defined in it:
...
traceback
    ...
    it lists the functions in the order in which they are stored in the
