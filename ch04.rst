...
In chapter 2 we were introduced to Python's built-in functions that get input
from the keyboard: ``raw_input`` and ``input``. Now let's look at these again
in greater depth.
...
GASP ( **G**\ raphics **A**\ PI for **S**\ tudents of **P**\ ython) will enable us to write programs involving graphics.
...
conditional statement
    ...
    condition. In Python the keywords ``if``, ``elif``, and ``else`` are used
...
body
    The block of statements in a compound statement that follows
    the header.
...
#. To better understand boolean expressions, it is helpful to construct truth
   ...
   they have the same truth table. The following Python script prints out the
   truth table for the any boolean expression in two variables: p and q:
...
#. Write a function named ``is_divisible_by_3`` that takes a single integer as
   ...
   otherwise. Now write a similar function named ``is_divisible_by_5``.
