...
wanted to print out.  Our definition was simple.  A string is some characters inside quotes.  In this chapter we explore strings in much more detail.
...
In order to make the iteration more general, we can use the ``len`` function to provide the bound for ``range``.  This is a very common pattern for traversing any sequence by position.	
...
.. activecode:: ch08_fun1
    ...
    def removeVowels(s):
        ...
        for x in s:
            if x not in vowels:
                sWithoutVowels = sWithoutVowels + x
