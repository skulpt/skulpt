...
value
    A number or string (or other thing to be named later) that can be stored in
    a variable or computed in an expression.
...
float
    ...
    Beware of rounding errors when you use ``float``\ s, and remember that they     are only approximate values.
...
assignment statement
    ...
    sides of the assignment statement is often confusing to new programmers. In 
    ...
    evaluated by the Python interpreter before assigning it to the name on the
    left.
...
variable name
    A name given to a variable. Variable names in Python consist of a sequence
    of letters (a..z, A..Z, and _) and digits (0..9) that begins with a letter. 
    In best programming practice, variable names should be choosen so that they 
    describe their use in the program, making the program *self documenting*.
