...
class TypeDefVisitor(EmitVisitor):
    ...
    def simple_sum(self, sum, name, depth):
        ...
        for i in range(len(sum.types)):
            ...
            type = sum.types[i]
            self.emit("astnodes.%s = function %s() {}" % (type.name, type.name), depth)
        self.emit("", depth)
...
class FunctionVisitor(PrototypeVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=1):
        ...
        emit("/** @constructor */")
        emit("astnodes.%s = function %s(%s)" % (name, name, argstr))
        emit("{")
...
class FieldNamesVisitor(PickleVisitor):
    ...
    def visitProduct(self, prod, name):
        if prod.fields:
            self.emit('astnodes.%s.prototype._astname = "%s";' % (name, cleanName(name)), 0)
            self.emit("astnodes.%s.prototype._fields = [" % name,0)
            c = 0
    ...
    def visitSum(self, sum, name):
        if is_simple(sum):
            for t in sum.types:
                self.emit('astnodes.%s.prototype._astname = "%s";' % (t.name, cleanName(t.name)), 0)
                self.emit('astnodes.%s.prototype._isenum = true;' % (t.name), 0)
        else:
            for t in sum.types:
                self.visitConstructor(t, name)

    def visitConstructor(self, cons, name):
        self.emit('astnodes.%s.prototype._astname = "%s";' % (cons.name, cleanName(cons.name)), 0)
        self.emit("astnodes.%s.prototype._fields = [" % cons.name, 0)
        if cons.fields:
            c = 0
