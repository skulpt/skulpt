    ...
    function generateTurtleModule(_target) {
        ...
        (function(proto) {
            ...
            proto.reset = function() {
                ...
                this._filling    = false;
                this._undoBuffer = [];
                this._speed      = 6/_config.scale;
                this._computed_speed = 10/_config.scale;
                this._colorMode  = 255;
            ...
            proto.$speed = function(speed) {
                if (speed !== undefined) {
                    this._speed   = Math.max(0, speed > 10 ? 0 : speed/_config.scale);
            ...
            proto.$dot = function(size, color, g, b, a) {
                ...
                size = (typeof size === "number") ?
                    Math.max(1, Math.abs(size) | 0) * _config.scale :
            ...
            proto.$pensize = proto.$width = function(size) {
                if (size !== undefined) {
                    this._size = size*_config.scale;
                    return this.addUpdate(undefined, this._shown, {size : size*_config.scale});
                }
        ...
        proto.$shapesize = function (width, height) {
            if (width && height) {
                _config.turtleWidth = width;
                _config.turtleHeight = height;
                this.addUpdate(undefined, this._shown);
            } 
        } 
        proto.$shapesize.minArgs = 2;
        proto.$shapesize.co_varnames = ["width", "height"];
        ...
        function Screen() {
            ...
            w = (_config.worldWidth || _config.width || getWidth()) / 2 / _config.scale;
            h = (_config.worldHeight || _config.height || getHeight()) / 2 / _config.scale;
        ...
        (function(proto) {
            ...
            proto.$setup = function(width, height, startX, startY) {
                ...
                this._width  = width / _config.scale;
                this._height = height / _config.scale;
    
                this._xOffset = (startX !== undefined && !isNaN(parseInt(startX))) ?
                    parseInt(startX) / _config.scale :
                    0;
    
                this._yOffset = (startY !== undefined && !isNaN(parseInt(startY))) ?
                    parseInt(startY) / _config.scale :
                    0;
        ...
        function clearLayer(context, color, image) {
            ...
            if (image) {
                context.drawImage(image, 0, 0, image.width*_config.scale, image.height*_config.scale);
            }
        ...
        function drawTurtle(state, context) {
            ...
            if (shape.nodeName) {
                ...
                var rw = _config.turtleWidth ? _config.turtleWidth : shape.naturalWidth;
                var rh = _config.turtleHeight ? _config.turtleHeight : shape.naturalHeight;
                var iw = rw * _config.scale;
                var ih = rh * _config.scale;
                var jw = shape.naturalWidth;
                var jh = shape.naturalHeight;
                context.drawImage(shape, 0, 0, jw, jh, -iw/2, -ih/2, iw, ih);
            }
            else {
