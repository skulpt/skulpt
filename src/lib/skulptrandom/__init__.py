...
class Random:
	...
	def seed(self, seed_value=None, algorithm_id=None):
		...
		if algorithm_id == CMRG:
			self.cmrg_seed(seed_value)
		elif algorithm_id == MRG:
			self.mrg_seed(seed_value)
#		else:
#			raise Exception("Unrecognized algorithm_id specified.")

		#	Initialize generator constants
		if self.algorithm_id == CMRG:
			self.a_1 = 0
			self.a_2 = 63308
			self.a_3 = 183326
			self.b_1 = 86098
	...
	def random(self):
		...
		elif self.algorithm_id == MRG:
			return self.random_mrg()
#		else:
#			raise Exception("Invalid algorithm_id setting " + str(self.algorithm_id))

	#Internal function to generate a random number with CMRG algorithm
	def random_cmrg(self):
#		if self.algorithm_id != CMRG:
#			raise Exception("Combined Multiple Recursive Generator is not the current algorithm.")
	...
	def random_mrg(self):
#		if self.algorithm_id != MRG:
#			raise Exception("Multiple Recursive Generator is not the current algorithm.")
