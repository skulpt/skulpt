function $builtinmodule() {
    const {
        ...
        ffi: { toPy, toJs, toPyFloat, toPyInt, toPyList, isTrue },
    ...
    const json = {
        ...
        __all__: toPyList(["dump", "dumps", "load", "loads", "JSONDecoder", "JSONDecodeError", "JSONEncoder"]),
        dump: proxyFail("dump"),
        load: proxyFail("load"),
    ...
    class JSONDecoder {
        ...
        white(s, idx) {
            const m = (idx === 0 ? s : s.substr(idx)).match(WHITESPACE);
            if (m !== null) {
                idx += m[0].length;
            }
            return idx;
