...
Sk.builtin.lng.prototype.nb$__int__ = function() {
    if (this.cantBeInt()) {
        return new Sk.builtin.lng(this);
    }

    return new Sk.builtin.nmber(this.toInt$(), Sk.builtin.nmber.int$);
};

Sk.builtin.lng.prototype.__index__ = new Sk.builtin.func(function(self) {
    return self.nb$__int__(self);
});

Sk.builtin.lng.prototype.nb$__float__ = function() {
    return new Sk.builtin.nmber(Sk.ffi.remapToJs(this), Sk.builtin.nmber.float$);
};
...
Sk.builtin.lng.prototype.ob$eq = function (other) {
    if (other instanceof Sk.builtin.int_ || other instanceof Sk.builtin.lng) {
        return this.longCompare(other) == 0 && !(other instanceof Sk.builtin.none); //jshint ignore:line
    } else {
        return Sk.builtin.NotImplemented.NotImplemented$;
    }
};
