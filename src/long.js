...
// based on Leemon Baird's public domain implementation (www.leemon.com)
// it's better not to think about how many processor-level operations this is causing!

// either a number (int) or an array (for construction from an array of bpe-elements)
Long$ = function(i)
{
    if (typeof i === "number")
        return Long$.fromInt(i, 0, 0);
    if (Object.prototype.toString.apply(i) !== '[object Array]') throw "assert";
    this.v = i;
};


Long$.bpe$ = 0; // bits per array element, calculated at load time to find mantissa
(function(){
 for (;
     (1 << (Long$.bpe$ + 1)) > (1 << Long$.bpe$);
     Long$.bpe$ += 1)
 { /* nothing */ }
 Long$.bpe$ >>= 1;
 }());
Long$.mask$ = (1 << Long$.bpe$) - 1; // & to chop to bpe bits
...
Long$.dup = function(x)
{
    var ret = new Long$(new Array(x.v.length));
    Long$.copy_(ret, x);
    return ret;
};

Long$.copy_ = function(x, y)
{
    var i;
    var k = Math.min(x.v.length, y.v.length);
    for (i = 0; i < k; ++i) x.v[i] = y.v[i];
    for (i = k; i < x.v.length; ++i) x.v[i] = 0;
};
