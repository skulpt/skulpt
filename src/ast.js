...
function setContext (c, e, ctx, n) {
    ...
    var exprName;
    var s;
    goog.asserts.assert(ctx !== astnodes.AugStore && ctx !== astnodes.AugLoad);
    s = null;
    exprName = null;
    ...
    switch (e.constructor) {
        ...
        case astnodes.ListComp:
            ...
            break;
        case astnodes.SetComp:
            exprName = "set comprehension";
            break;
        case astnodes.DictComp:
...
function astForTryStmt (c, n) {
    ...
    if (nexcept > 0) {
        ...
        exceptSt = new astnodes.TryExcept(body, handlers, orelse, n.lineno, n.col_offset);
...
function astForDottedName (c, n) {
    ...
    e = new astnodes.Name(id, astnodes.Load, lineno, col_offset);
    for (i = 2; i < NCH(n); i += 2) {
...
function astForWithItem (c, n, content) {
    ...
    if (NCH(n) == 3) {
        ...
        setContext(c, optional_vars, astnodes.Store, n);
    }

    return new astnodes.With_(context_expr, optional_vars, content, n.lineno, n.col_offset);
}
...
function astForListcomp (c, n) {
    ...
    for (i = 0; i < nfors; ++i) {
        REQ(ch, SYM.list_for);
        forch = CHILD(ch, 1);
        t = astForExprlist(c, forch, astnodes.Store);
...
function astForForStmt (c, n) {
    ...
    _target = astForExprlist(c, nodeTarget, astnodes.Store);
    if (NCH(nodeTarget) === 1) {
        target = _target[0];
    }
    else {
    ...
    if (CHILD(n, 0).type === TOK.T_LPAR) {
        ...
        if (NCH(n) === 1) {
            ...
            for (j = 0; j < slices.length; ++j) {
                slc = slices[j];
                goog.asserts.assert(slc.constructor === astnodes.Index && slc.value !== null && slc.value !== undefined);
                elts[j] = slc.value;
            }
            e = new astnodes.Tuple(elts, astnodes.Load, n.lineno, n.col_offset);
...
function astForComprehension(c, n) {
    ...
    for (i = 0; i < nfors; ++i) {
        ...
        expression = astForExpr(c, CHILD(n, 3));
        if (NCH(forch) === 1) {
            comp = new astnodes.comprehension(t[0], expression, []);
        } else {
            comp = new astnodes.comprehension(new astnodes.Tuple(t, astnodes.Store, n.lineno, n.col_offset), expression, []);
        }
        if (NCH(n) === 5) {
            n = CHILD(n, 4);
...
function astForIterComp(c, n, type) {
    ...
    if (type === COMP_GENEXP) {
        return new astnodes.GeneratorExp(elt, comps, n.lineno, n.col_offset);
    } else if (type === COMP_SETCOMP) {
        return new astnodes.SetComp(elt, comps, n.lineno, n.col_offset);
    }
}
...
function astForDictComp(c, n) {
    ...
    return new astnodes.DictComp(key, value, comps, n.lineno, n.col_offset);
}
    ...
    switch (ch.type) {
        ...
        case TOK.T_LBRACE:
            ...
            if (n.type === TOK.T_RBRACE) {
                //it's an empty dict
                return new astnodes.Dict([], null, n.lineno, n.col_offset);
            } 
                ...
                return new astnodes.Set(elts, n.lineno, n.col_offset);
            } 
        ...
        case TOK.T_BACKQUOTE:
            ...
            return new astnodes.Repr(astForTestlist(c, CHILD(n, 1)), n.lineno, n.col_offset);
        default:
    ...
    if (n.type === SYM.small_stmt) {
        ...
        switch (n.type) {
            ...
            case SYM.debugger_stmt:
                return new astnodes.Debugger_(n.lineno, n.col_offset);
            default:
