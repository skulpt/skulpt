...
Before we do that, let's just review a few ideas related to variables and assignment.
...
the value that ``bruce`` refers to.
...
value.  The new value of x is the old value of x plus 1.
...
.. note::
	...
	Introduction of the while statement causes us to think about the types of iteration we have seen.  The ``for`` statement will always iterate through a sequence of values.  Since we know that it will iterate once for each value in the collection, it is often said that a ``for`` loop creates a
	**definite iteration** because we definitely know how many times we are going to iterate.  On the other
	...
	call **indefinite iteration**.  Indefinite iteration simply means that we don't know how many times we will repeat but eventually the condition controlling the iteration will fail and it will stop. (Unless we have an infinite loop which is of course a problem)
...
loop one has to control the loop variable yourself.  You give it an initial value, test
...
This function demonstrates an important pattern of computation called a **counter** (note that it is
