...
class ReTests(unittest.TestCase):
    ...
    # #@TODO not supported this type of regex
    ...
    def test_symbolic_groups(self):
        ...
        # Conditional groups are unsupported; retain the identifier/backreference checks.
        re.compile('(?P<µ>x)(?P=µ)')
        re.compile('(?P<𝔘𝔫𝔦𝔠𝔬𝔡𝔢>x)(?P=𝔘𝔫𝔦𝔠𝔬𝔡𝔢)')
    ...
    def test_re_split(self):
        ...
        # python is weird here - skulpt behaves well enough and more like python 2
        for sep, expected in [
            (':*', ['', '', 'a', '', 'b', '', 'c', '']),
            ('(?::*)', ['', '', 'a', '', 'b', '', 'c', '']),
            ('(:*)', ['', ':', '', '', 'a', ':', '', '', 'b', '::', '', '', 'c', '', '']),
    ...
    def test_re_groupref(self):
        ...
        self.assertEqual(re.match(r'^(\|)?([^()]+)\1?$', 'a').groups(),
                         (None, 'a'))
        # the following two are matches in skulpt and in javascritpt
        # self.assertIsNone(re.match(r'^(\|)?([^()]+)\1$', 'a|'))
        # self.assertIsNone(re.match(r'^(\|)?([^()]+)\1$', '|a'))
    ...
    def test_lookbehind(self):
        ...
        # @TODO - but javascript doesn't support this type of syntax so maybe not todo
        # # Conditional group reference.
    ...
    def test_ignore_case_range(self):
        ...
        # self.assertIsNone(re.match(r'[9-A]', '_', re.I)) # throws an error
        ...
        self.assertTrue(re.match(r'[\u0400-\u042f]', '\u0400', re.I))
        # these 4 tests should have r' but does not compile with r'
        self.assertTrue(re.match('[\U00010428-\U0001044f]', '\U00010428', re.I))
        self.assertTrue(re.match('[\U00010428-\U0001044f]', '\U00010400', re.I))
        self.assertTrue(re.match('[\U00010400-\U00010427]', '\U00010428', re.I))
        self.assertTrue(re.match('[\U00010400-\U00010427]', '\U00010400', re.I))
    ...
    def test_sre_character_literals(self):
        ...
        self.assertTrue(re.match(r"\08", chr(0)+"8"))
        self.assertTrue(re.match(r"\01", chr(1)))
        self.assertTrue(re.match(r"\018", chr(1)+"8"))
    ...
    def test_sre_character_class_literals(self):
        ...
        self.assertTrue(re.match("[\U0001d49c-\U0001d4b5]", "\U0001d49e"))
    ...
    def test_scoped_flags(self):
        # Scoped flags are explicitly unsupported on Skulpt. Keep CPython behavior here for future support.
    ...
    def test_match_repr(self):
        for string in '[abracadabra]', S('[abracadabra]'):
            ...
            self.assertTrue(re.fullmatch(pattern, repr(m)))
        ...
        self.assertTrue(re.fullmatch(pattern, repr(first)))
        # self.assertRegex(repr(first), pattern)
        pattern = r"<(%s\.)?%s object; span=\(3, 5\), match='bb'>" % (
            type(second).__module__, type(second).__name__
        )
        self.assertTrue(re.fullmatch(pattern, repr(second)))
        # self.assertRegex(repr(second), pattern)

    def test_zerowidth(self):
        # these don't work in skulpt
        ...
        # skulpt/javascript matches : rather than ::
        # self.assertEqual(re.sub(r"\b|:+", "-", "a::bc"), '-a---bc-')
    ...
    def test_bug_2537(self):
        ...
        for outer_op in ('{0,}', '*', '+', '{1,187}'):
            for inner_op in ('{0,}', '*', '?'):
                ...
                # self.assertEqual(m.group(1), "") # js doesn't do this
...
#         self.assertEqual(out.getvalue(), dump)
    ...
    def test_error(self):
        ...
        # Bytes patterns remain unsupported.
        # Multiline pattern
...
######## test methods ######""" Unit test for re module"""

class ReTestsBasic(unittest.TestCase):
