...
UserList = list # CPython's test tests two different list types
...
class TestBisect:
    module = bisect # this is an adaption of how CPython's test does it
    def setUp(self):
...
class TestErrorHandling:
    ...
    def test_len_only(self):
        for f in (self.module.bisect_left, self.module.bisect_right,
                  self.module.insort_left, self.module.insort_right):
#py3            self.assertRaises(TypeError, f, LenOnly(), 10)
            self.assertRaises(StandardError, f, LenOnly(), 10)

    def test_get_only(self):
        for f in (self.module.bisect_left, self.module.bisect_right,
                  self.module.insort_left, self.module.insort_right):
# py3            self.assertRaises(TypeError, f, GetOnly(), 10)
            self.assertRaises(StandardError, f, GetOnly(), 10)
