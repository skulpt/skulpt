 ...
 * Or require and call run({ verbose: false }) for summary only
 */

function run(options = {}) {
    const { verbose = true } = options;

    // Load Skulpt if not already loaded (sets up global Sk needed for Unicode classes)
    ...
    const { parseAndConvert: _parseAndConvert } = require('../src/lib/re.js');

    // Wrapper that defaults to ASCII mode for simpler test expectations
    function parseAndConvert(pattern, opts = {}) {
        return _parseAndConvert(pattern, { asciiMode: true, ...opts });
    }
    ...
    describe('Character classes', () => {
        it('should handle ] at start of character class', () => {
            const result = parseAndConvert('[]abc]');
            // In JS unicode mode, ] must be escaped inside character classes
            expect(result.pattern).toBe('[\\]abc]');
        });
    ...
    console.log(`re_parser tests: ${passed} passed, ${failed} failed`);
...
// Export for use by testwrapper
module.exports = { run };

// Run directly if this is the main module
if (require.main === module) {
    const { failed } = run({ verbose: true });
